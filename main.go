...
func main() {
	...
	http.HandleFunc(
		"/api/submit-subscription-payment",
		submitSubscriptionPaymentHandler,
	)
	...
	http.HandleFunc(
		"/api/storyteller/upload",
		uploadStoryJSONHandler,
	)
	...
	http.HandleFunc(
		"/api/admin/designs",
		adminGetDesignsHandler,
	)

	http.HandleFunc(
		"/api/admin/approve",
		adminApproveDesignHandler,
	)

	http.HandleFunc(
		"/api/admin/reject",
		adminRejectDesignHandler,
	)

	http.HandleFunc(
		"/api/admin/delete-design",
		adminDeleteDesignHandler,
	)

	http.HandleFunc(
		"/api/admin/stories",
		adminGetStoriesHandler,
	)

	http.HandleFunc(
		"/api/admin/approve-story",
		adminApproveStoryHandler,
	)

	http.HandleFunc(
		"/api/admin/reject-story",
		adminRejectStoryHandler,
	)

	http.HandleFunc(
		"/api/admin/delete-story",
		adminDeleteStoryHandler,
	)
	...
	http.HandleFunc(
		"/api/admin/buyers",
		adminGetBuyersHandler,
	)

	http.HandleFunc(
		"/api/admin/sellers",
		adminGetSellersHandler,
	)

	http.HandleFunc(
		"/api/admin/storytellers",
		adminGetStorytellersHandler,
	)
	...
	http.HandleFunc(
		"/api/admin/add-admin",
		adminAddAdminHandler,
	)

	http.HandleFunc(
		"/api/admin/approve-subscription",
		adminApproveSubscriptionHandler,
	)
	...
	fmt.Printf(
		"Seva inaanza kusikiliza kwenye bandari (port) %s...\n",
		port,
	)
...
func initDB() {
	if err := os.MkdirAll("./uploads", 0755); err != nil {
		log.Fatalf(
			"Imeshindikana kutengeneza folder la uploads: %v",
			err,
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"Imeshindikana kutengeneza jedwali la app_accounts: %v",
			err,
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"Imeshindikana kutengeneza jedwali la designs: %v",
			err,
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"Imeshindikana kutengeneza jedwali la stories: %v",
			err,
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"Imeshindikana kutengeneza jedwali la orders: %v",
			err,
		)
	}
	...
	db.Exec(`
		ALTER TABLE app_accounts
		ADD COLUMN IF NOT EXISTS payment_name TEXT DEFAULT '';
	`)

	// =====================================================
	// MUHIMU:
	// HADITHI ZENYE BEI 0 ZIWE FREE.
	//
	// Hii inasaidia kurekebisha hadithi za zamani
	// ambazo zinaweza kuwa na price = 0 lakini
	// is_paid = TRUE kwa makosa.
	// =====================================================
	...
	if err != nil {
		log.Printf(
			"Onyo: Imeshindikana kusafisha hadithi za bure: %v",
			err,
		)
	}
}
...
func signupHandler(w http.ResponseWriter, r *http.Request) {
	...
	if rawIDImage != "" &&
		strings.HasPrefix(rawIDImage, "data:") {

		if savedURL, saveErr :=
			saveBase64Media(rawIDImage); saveErr == nil {
	...
	if username == "" || password == "" {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Jaza jina la mtumiaji na nenosiri!",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Jina hili la mtumiaji linatumika tayari!",
			},
		)
...
func profileHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow(`
		SELECT
			...
			COALESCE(id_type, ''),
			COALESCE(id_number, ''),
			COALESCE(id_image_url, ''),
			COALESCE(rejection_reason, ''),
			COALESCE(subscription_status, 'free'),
			trial_ends_at,
			COALESCE(payment_phone, ''),
			COALESCE(payment_name, '')
		FROM app_accounts
		WHERE username = $1
	`, username).Scan(
		&u.ID,
...
func submitSubscriptionPaymentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" ||
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Tafadhali jaza namba ya simu na jina ulilofanyia malipo!",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kutuma taarifa za malipo",
			},
		)
...
func adminApproveSubscriptionHandler(w http.ResponseWriter, r *http.Request) {
	...
	if currentTrial.Valid &&
		currentTrial.Time.After(baseTime) {

		baseTime = currentTrial.Time
	}

	newTrial := baseTime.Add(
		30 * 24 * time.Hour,
	)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kusasisha usajili",
			},
		)
...
func uploadDesignJSONHandler(w http.ResponseWriter, r *http.Request) {
	...
	var designerName, location, vendorPhone, videoURL string
	...
	if designerName != "" {
		...
		if ok, errMsg :=
			checkSubscriptionAndVerification(
				designerName,
			); !ok {
	...
	if strings.HasPrefix(img1, "data:") {
		...
		if url, saveErr :=
			saveBase64Media(img1); saveErr == nil {
	...
	if strings.HasPrefix(img2, "data:") {
		...
		if url, saveErr :=
			saveBase64Media(img2); saveErr == nil {
	...
	if strings.HasPrefix(img3, "data:") {
		...
		if url, saveErr :=
			saveBase64Media(img3); saveErr == nil {
	...
	if strings.HasPrefix(img4, "data:") {
		...
		if url, saveErr :=
			saveBase64Media(img4); saveErr == nil {
	...
	json.NewEncoder(w).Encode(
		map[string]interface{}{
			"success": true,
			"message": "Bidhaa imechapishwa kwenye Soko Kuu kwa mafanikio!",
		},
	)
}
...
func updateDesignJSONHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kusasisha",
			},
		)
	...
	json.NewEncoder(w).Encode(
		map[string]interface{}{
			"success": true,
			"message": "Imesasishwa kikamilifu!",
		},
	)
}
...
func buyDesignHandler(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(
		map[string]interface{}{
			"success": true,
			"message": "Oda imepokelewa!",
		},
	)
}

// =========================================================
// HADITHI - GET ALL APPROVED STORIES
// =========================================================
...
func getMyStoriesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(
			w,
			"Imeshindikana kusoma hadithi zako",
			http.StatusInternalServerError,
		)
...
func uploadStoryJSONHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Hii inasaidia frontend inayotuma:
	// free / paid
	...
	if title == "" {
		...
		storytellerName =
			strings.TrimSpace(
				r.FormValue("storyteller_name"),
			)

		if storytellerName == "" {
			storytellerName =
				strings.TrimSpace(
					r.FormValue("designer_name"),
				)
		}
		...
		storyType =
			strings.ToLower(
				strings.TrimSpace(
					r.FormValue("story_type"),
				),
			)

		// =================================================
		// FREE/BURE ALWAYS OVERRIDES
		// =================================================
	...
	if title == "" ||
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Tafadhali jaza kichwa na maudhui ya hadithi.",
			},
		)
	...
	if isPaid &&
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Hadithi ya kulipia lazima iwe na bei zaidi ya Tsh 0.",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kuhifadhi hadithi: " +
					err.Error(),
			},
		)
	...
	message :=
		"Hadithi ya Bure imechapishwa kikamilifu."

	storyTypeResponse := "free"

	if isPaid {

		message =
			"Hadithi ya Kulipia imechapishwa kikamilifu."

		storyTypeResponse = "paid"
	}
...
func deleteMyStoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kufuta hadithi",
			},
		)
...
func adminDeleteDesignHandler(w http.ResponseWriter, r *http.Request) {
	...
	if designID == "" {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "ID ya bidhaa inahitajika!",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kufuta bidhaa kwenye database",
			},
		)
...
func adminRejectStoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if reason == "" {
		reason =
			"Haikutimiza vigezo vya hadithi."
	}
...
func adminDeleteStoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if storyID == "" {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "ID ya hadithi inahitajika!",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kufuta hadithi kwenye database",
			},
		)
...
func adminDeleteUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if userID == "" {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "ID ya mtumiaji inahitajika!",
			},
		)
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(
			map[string]interface{}{
				"success": false,
				"message": "Imeshindikana kufuta mtumiaji kwenye database",
			},
		)
